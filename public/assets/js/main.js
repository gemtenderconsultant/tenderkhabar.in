...
$('.dropdown-toggle').on('shown.bs.dropdown', function () {
    ...
    $('.searchsubcategory').delayKeyup(function(){
            $('.tender-listing-checkbox-grid').html('');
            countsubcat = 1;
...
$('body').on('click','.filter_apply',function(e){
        ...
        if(data == "subcategory" || data == 'mobile_apply'){
            ...
            if (s1 > 0) {
                ...
                $('.chk_selected_login_subcategory').show();
                 console.log("subcategory apply button call"+subcategory);
                ...
                $("input[name='input_s_subcategory']").val(subcategory.join(","));    
               console.log($("input[name='input_s_subcategory']").val());
            }else{
                $('.subcategory_count').html('');
                $('.list_selected_login_subcategory').html('');
                $("input[name='input_s_subcategory']").val('');
 ...
 $('body').on('click', '.btn_clear_selected_item', function () {
    ...
    var data = $(this).attr('data');
    console.log("filter is remove"+data);
...
function tabloadcategory(){
     console.log("loading tabloadcategory");
    if($('.tender-listing-checkbox-grid').find('.searchkeyword_tr').length < 10){
        ...
        $.each(city_list, function(key, value){
			if(jQuery.inArray(value.stateid, selectedState) !== -1)			
			{
              ...
              if($(".cityid[data='"+value.text+"']").length == 0){  
                ...
                //     '</div>');
                    $('.city_list').append('<label class="tender-listing-checkbox-item">'+'<input class="cityid" type="checkbox" value="'+value.id+'" id="sFilters_city_'+value.id+'" name="Filters[city][]" data="'+value.text+'" '+ischecked+'>'+value.text+'</label>');
              }
			}
		});
	}else{
...
function get_subcategory_data(){
    ...
    var output = ''; 
    		if(searchField === ''){
    		    $.each(subcategory_list,function(i,item){
                    ...
                    if($(".subcategoryid[data='"+item.subcategoryname+"']").length == 0){    
                       output +='<div class="list_category"><label class="tender-listing-checkbox-item" data="{{$selectedsubcategoryid}}"><input '+ischecked+' class="subcategoryid" data-title="'+item.subcategoryname+'" '+ischecked+' type="checkbox" value="'+item.subcategoryid+'" id="sFilters_subcategory_'+item.subcategoryid+'" data="'+item.subcategoryname+'" name="Filters[subcategoryid][]">'+item.subcategoryname+'</label></div>';
                       countsubcat++;
                    }   
    		    });
    		}else{
          ...
          //console.log("cc "+i);
    $('.tender-listing-checkbox-grid').append(output);
	if(busy_subcategory){
        busy_subcategory = false;
    }
}
    ...
    var output = ''; 
        ...
        $.each(keyword_list,function(i,item){
            ...
            if($(".keywordid[data='"+item.name+"']").length == 0){    
                output +='<div class="keyword_list"><label class="tender-listing-checkbox-item"><input '+ischecked+' class="keywordid" data-title="'+item.name+'" '+ischecked+' type="checkbox" value="'+item.name+'" id="sFilters_keyword_'+item.id+'" data="'+item.name+'" name="Filters[keyword][]">'+item.name+'</label></div>';
                countkeyword++;
            }
        });
        }else{
    		...
    		if(searchField === ''){
    		     $.each(dipartment_list,function(i,item){
                    ...
                    if($(".agencyid[data-id='"+item.id+"']").length == 0){    
                       output +='<div class="list-department"><label class="tender-listing-checkbox-item"><input '+ischecked+' class="agencyid" data-title="'+item.text+'" '+ischecked+' type="checkbox" value="'+item.id+'" id="dipartment_'+item.id+'" data="'+item.text+'" name="Filters[agency][]">'+item.text+'</label></div>';
                       countdept++;
                    }     
    		     });
    		}else{
