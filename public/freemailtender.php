...
//$emailUser = "AKIAXZEFIAAVNGAVXOVR";
//$emailPassword = "BEpVPPV+rB9zgtfF2X729h40mTkr8st2oxdTj/3rqBS6";
...
//  $sql_check_holiday = "select * from holidays_for_email where holiday_date='$dates'";
// $query_check_holiday = mysqli_query($dbh1,$sql_check_holiday);
// $result_check_holiday = mysqli_fetch_assoc($query_check_holiday);

// if(date('w') != 0 && empty($result_check_holiday)){
if(date('w') != 0){
...
if ($my_users === false) {
    // The query failed. Print the SQL query and the database error, then stop the script.
    die("SQL Error: " . mysqli_error($dbh2) . "<br><br>Failing Query: " . $sql1);
}
...
foreach ($paid_user_tender as $u_key => $u_val) {
    ...
    $sql .= " AND t.submitdate >= '" . date('Y-m-d') . "' group by t.ourrefno ORDER BY t.ourrefno ASC LIMIT 100"; //t.state_name
      ...
      if (mysqli_num_rows($resultchk) > 0) {
        ...
        //echo 'not found';
            ...
            /*****************mail send using smtp for domain email id***********************/
             ...
             $emailstatus = 0; //for free 1 for paid  
             //echo $email_sent_status;die();
            if ($email_sent_status == 1) {
                
                $tender_id = implode(',', $tender_array);
                $live_tender_count = 0;
                if($mailsent == "Message has been sent successfully" || $status == 1){
