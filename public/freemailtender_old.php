...
function email_phpmailer($emailid = array(), $subject, $body,$username,$password,$from,$fromname,$host){
    ...
    $mail->AddCC('tenderkhabar2@gmail.com');
    $mail->isHTML(true);
...
$full_details_path = "https://tenderkhabar.in/EmailTenderDetails";
$logo_url = "https://tenderkhabar.in/assets/img/logo.png";
$mailto = "sales@gemtenderconsultant.in";
$infomailto ="sales@gemtenderconsultant.in";
$website ="www.tenderkhabar.in";
$phone ="(+91) 9824 89 5546";

$tablename = "mail_live_tenders";
...
$sql_tender_sent = 'select sent_date from tenders_date where flag="f" order by sent_date desc limit 0,1';
...
$sql1 = 'SELECT user.id as u_id,user.email,user.alt_email,user.status,user.name,user.company_name,userproduct.* from users as user LEFT JOIN userproduct on user.id=userproduct.user_id  where user.is_tender=1 and userproduct.todate>="' . $dates . '" and user.status="Free" AND user.id NOT IN(SELECT user_id FROM userinfo WHERE userinfo.tender_date="' . $dates . '") group by user.id order by user.id asc';
...
while ($m = mysqli_fetch_assoc($my_users)) {
    ...
    $sql_tender_dates = "insert into tenders_date (sent_date,flag) values ('$dates','f')";
...
foreach ($paid_user_tender as $u_key => $u_val) {
    ...
    if($u_val['productid'] == "" && $u_val['categoryid'] == "" && $u_val['subcategoryid'] == "" && $u_val['keyword'] == ""){
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="2">
          <tbody>
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="min-width:100%">
                        <tbody>
                            <tr>
                                ...
                                <td align="right">
                                 <img src="'.$logo_url.'" width="100" alt="'.$fromname.'">
                                </td>
                            </tr>
                        </tbody>
                    </table>
            ...
            <tr>
              <td>
                <table cellspacing="0" cellpadding="0" style="font-family:Arial,Helvetica,sans-serif;border:1px solid #011736">
                  <thead>
                    <tr style="background-color:#011736">
    ...
    $sql .= " AND t.submitdate >= '" . date('Y-m-d') . "' group by t.ourrefno ORDER BY t.ourrefno ASC LIMIT 1000"; //t.state_name
    $sql1 .= " AND t.submitdate >= '" . date('Y-m-d') . "' group by t.ourrefno limit 0,100";
    
    //$my_paid_mail_date = "2020-11-06";  
    //$sql .= " AND t.dt = '" . $my_paid_mail_date . "' AND t.submitdate >= '" . date('Y-m-d') . "' group by t.ourrefno";
    //$sql1 .= " AND t.dt = '" . $my_paid_mail_date . "' AND t.submitdate >= '" . date('Y-m-d') . "' group by t.ourrefno limit 0,100";
...
$my_city_name = ucwords(trim($my_city_name));  
          ...
          $body .=' <tr>
              <td width="65%" style="border-right:1px solid #a69e9e;border-bottom:1px solid #011736;padding-bottom:5px">
              ...
              <td style="width:20%;color:#41515e;text-align:center;border-bottom:1px solid #011736;padding-bottom:5px;border-right:1px solid #a69e9e">
              ...
              <td style="width:15%;color:#41515e;text-align:center;border-bottom:1px solid #011736;padding-bottom:5px">
                <p style="font-family:sans-serif;font-size:13px;font-stretch:normal;line-height:23px">
...
echo $body;die();
    ...
    $from_email = $infomailto;
    // More headers
    $headers .= 'From: '.$infomailto.'' . "\r\n";
      ...
      if (mysqli_num_rows($resultchk) > 0) {
        ...
        //echo 'not found';
            ...
            if ($email_sent_status == 1) {
                ...
                $live_tender_count = 0;
                $sql = "insert into userinfo (user_id,tender_date,status,is_live_tender_setting,total_fresh,total_live,support_executive,no_tender_available) values ('" . $u_val['user_id'] . "','" . date('Y-m-d') . "','0','0','" . $tender_count . "','" . $live_tender_count . "','0','0')";
...
$subject = 'Free User '.$fromname.' Tender Mail '.date('Y-m-d')." Today";
$body = $comment.' '.date('Y-m-d H:i:s')." Today!";

$sql_remaining_send_email = "SELECT COUNT(*) as total from users as user LEFT JOIN userproduct on user.id=userproduct.user_id where user.is_tender=1 and userproduct.todate>='".$dates."' and user.status='Free' AND user.id NOT IN(SELECT user_id FROM userinfo WHERE userinfo.tender_date='".$dates."')";
...
if ($count_remaining_send_email == 0) { 
        ...
        $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
        // More headers
        $headers .= 'From: '.$fromname.' <'.$infomailto.'>' . "\r\n";
