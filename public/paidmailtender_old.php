...
// Log file path
$log_file = '/home/u920518903/domains/tenderkhabar.in/public_html/logs/daily_send_mail.log';
...
//writeLog("Script started", $log_file);
//echo "asdasd";die();

// Example: Simulating an error
try {
function email_phpmailer($emailid= array(), $subject, $body,$username,$password,$from,$fromname,$host, $log_file){
    $mail = new PHPMailer;
    ...
    //my code
    //writeLog("ERROR: Failed to send email to $email", $log_file);
    //writeLog("Actual Error: $issent", $log_file);
    }
    else
    {
    $issent = "Message has been sent successfully";
    //my code
    //writeLog("SUCCESS: Email sent to $email", $log_file);
    }
    return $issent;
}
...
$sitename = "tenderkhabar.in";

$finish_email_to = 'gautish.marvelinfo@gmail.com';
...
$query_tender_sent = mysqli_query($dbh2,$sql_tender_sent);
...
//$sql1 = 'SELECT user.user_id as u_id,user.email,user.first_time_email,user.secondary_email,user.user_active_indicator,user.user_first_name,user.user_last_name,company.company_name,userproduct.*,usersubscription.* from user LEFT JOIN userproduct on user.user_id=userproduct.user_id LEFT JOIN usersubscription on user.user_id=usersubscription.user_id LEFT JOIN company on user.user_id=company.user_id where user.status="Active" and user.is_deleted="0" and user.flag="F" and user.user_active_indicator="Paid" and tenderuser=1 and usersubscription.status="Active" and usersubscription.todate>="' . $dates . '" group by user.username order by user.user_id ';
//$sql1 = 'SELECT user.user_id as u_id,user.email,user.first_time_email,user.secondary_email,user.user_active_indicator,user.user_first_name,user.user_last_name,company.company_name,userproduct.*,usersubscription.* from user LEFT JOIN userproduct on user.user_id=userproduct.user_id LEFT JOIN usersubscription on user.user_id=usersubscription.user_id LEFT JOIN company on user.user_id=company.user_id where ((user.status="Active" and usersubscription.todate>="' . $dates . '") OR (user.bidding_status="Live")) and user.is_deleted="0" and user.flag="F" and user.user_active_indicator="Paid" and tenderuser=1 and usersubscription.status="Active" group by user.username order by user.user_id ';
...
//$sql1 = 'SELECT user.id as u_id,user.email,user.alt_email,user.status,user.name,user.company_name,userproduct.* from users as user LEFT JOIN userproduct on user.id=userproduct.user_id  where user.is_tender=1 and userproduct.todate>="' . $dates . '" and user.status="Paid" AND user.id=1 AND user.id NOT IN(SELECT user_id FROM userinfo WHERE userinfo.tender_date="' . $dates . '") group by user.id order by user.id asc';

//echo $sql1;die();
$my_users = mysqli_query($dbh2,$sql1);
//$sql1 = 'SELECT user.user_id as u_id,user.email,user.first_time_email,user.secondary_email,user.user_active_indicator,user.user_first_name,user.user_last_name,company.company_name,userproduct.*,usersubscription.* from user LEFT JOIN userproduct on user.user_id=userproduct.user_id LEFT JOIN usersubscription on user.user_id=usersubscription.user_id LEFT JOIN company on user.user_id=company.user_id where ((user.status="Active" and usersubscription.todate>="' . $dates . '") OR (user.bidding_status="Live")) AND user.user_id=51232 and user.is_deleted="0" and user.flag="F" and user.user_active_indicator="Paid" and tenderuser=1 and usersubscription.status="Active" AND user.user_id NOT IN(SELECT user_id from donotdisturb WHERE ((CURDATE() BETWEEN donotdisturb.from_date AND donotdisturb.to_date) AND donotdisturb.paid_mail=1)) AND user.user_id NOT IN(SELECT user_id FROM userinfo WHERE userinfo.tender_date="' . $dates . '") group by user.username order by user.user_id ';

//$dates = '2021-09-24';
//$sql1 = 'SELECT user.user_id as u_id,user.email,user.first_time_email,user.secondary_email,user.user_active_indicator,user.user_first_name,user.user_last_name,company.company_name,userproduct.*,usersubscription.* from user LEFT JOIN userproduct on user.user_id=userproduct.user_id LEFT JOIN usersubscription on user.user_id=usersubscription.user_id LEFT JOIN company on user.user_id=company.user_id where user.user_id=49340 AND ((user.status="Active" and usersubscription.todate>="' . $dates . '") OR (user.bidding_status="Live")) and user.is_deleted="0" and user.flag="F" and user.user_active_indicator="Paid" and tenderuser=1 and usersubscription.status="Active" group by user.username order by user.user_id ';
...
while ($m = mysqli_fetch_assoc($my_users)) {
    ...
    $sql_tender_dates = "insert into tenders_date (sent_date,flag) values ('$dates','p')";
...
foreach ($paid_user_tender as $u_key => $u_val) {
    ...
    if($u_val['productid'] == "" && $u_val['categoryid'] == "" && $u_val['subcategoryid'] == "" && $u_val['keyword'] == ""){
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="2">
          <tbody>
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="min-width:100%">
                        <tbody>
                            <tr>
                                ...
                                <td align="right">
                                 <img src="'.$logo_url.'" width="100" alt="'.$sitename.'">
                                </td>
                            </tr>
                        </tbody>
                    </table>
            ...
            <tr>
              <td>
                <table cellspacing="0" cellpadding="0" style="font-family:Arial,Helvetica,sans-serif;border:1px solid #002e3a">
                  <thead>
                    <tr style="background-color:#002e3a">
        ...
        <span style="margin-right:10px;font-weight:bold;font-size:20px;">Todays! No Fresh Tender for You.</span>
            ...
            <tr>
              <td style="font-family:sans-serif;color:#464545;padding:15px 0 0 0">If you have any query or problem, please feel free to contact our support team:<br><br>You have received this mail from '.$sitename.' because you have subscribed to our Tender alert service.<br>
                <br>If you do not want to receive this mailer, 
                 <a href="" style="color: #ed7e2c;" title="TenderKhabar" target="_blank">Unsubscribe</a>
              </td>
            </tr>
            <tr>
              <td>
    ...
    /*$sql = "SELECT t.*,tc.categoryid,tc.categoryid,tc.subcategory,'India' as country,s.id,s.name,a.agencyname from live_tenders as t
                LEFT JOIN livetendercategory as tc ON t.ourrefno = tc.ourrefno                      
                LEFT JOIN state s ON s.id = t.stateid 
                LEFT JOIN agency a ON a.agencyid = t.agencyid 
                where t.ourrefno = tc.ourrefno "; */
    ...
    if (!empty($u_val['productid']) || !empty($u_val['categoryid']) || !empty($u_val['subcategoryid']) || !empty($u_val['exe_productid']) || !empty($u_val['exe_categoryid']) || !empty($u_val['exe_subcategoryid'])) {
            ...
            //$cat_arr=$_POST['category'];
        	$cat_ids = $u_val['categoryid'];
        	
        	//$subcat_arr=$_POST['subcategory'];
        	$subcat_ids = $u_val['subcategoryid'];
        	
        	//$eproduct_arr=$_POST['eproduct'];
        	$eproduct_ids = $u_val['exe_productid'];
        	
            //$ecat_arr=$_POST['ecategory'];
        	$ecat_ids = $u_val['exe_categoryid'];
        	
        	//$esubcat_arr=$_POST['esubcategory'];
    ...
    if (!empty($u_val['refine_keyword']) && !empty($u_val['refine_keyword'])) {
        //$sql .= " AND t.Work LIKE '%" . $u_val['refine_keyword'] . "%'";
        ...
        for ($j = 0; $j < count($userexcludingkeyword); $j++) {
            if($userexcludingkeyword[$j] != ""){
                if (count($userexcludingkeyword) == 1) {
                    $sql .= " AND t.Work NOT LIKE '%" . $userexcludingkeyword[$j] . "%'";
                }
                if (count($userexcludingkeyword) > 1) {
                    if ($j == 0) {
                        $sql .= " AND (t.Work NOT LIKE '%" . $userexcludingkeyword[$j] . "%' AND ";
                    } else if ($j == (count($userexcludingkeyword) - 1)) {
    
                        $sql .= " t.Work NOT LIKE '%" . $userexcludingkeyword[$j] . "%')";
                    } else {
                        $sql .= " t.Work NOT LIKE '%" . $userexcludingkeyword[$j] . "%' AND ";
                    }
                }
            }
    ...
    if ($u_val['city'] != '') {      
        ...
        /*$nearby_city = array();
        $nearby_city_list = array();
        foreach ($city_explode as $k2 => $j2) {
            $my_nearby_city = "select cities from nearby_city where cities like '%" . $j2 . "%'";
            $my_nearby_list = mysqli_query($dbh1,$my_nearby_city);
            while ($k_city = mysqli_fetch_assoc($my_nearby_list)) {
                $nearby_city_list[] = $k_city;
            }
    ...
    $sql1 = $sql;
    //$sql2 = $sql;
    //$my_paid_mail_date = '2019-12-25';
    //$sql .= " AND t.dt > '" . $my_paid_mail_date . "' AND t.submitdate >= '" . date('Y-m-d') . "' group by t.ourrefno ORDER BY t.state_name ASC";
    //$sql1 .= " AND t.dt <= '" . $my_paid_mail_date . "' AND t.submitdate >= '" . date('Y-m-d') . "' group by t.ourrefno limit 0,100";
    
    //$sql .= " AND t.submitdate >= '" . date('Y-m-d') . "' group by t.ourrefno ORDER BY t.ourrefno ASC LIMIT 4000"; //t.state_name
    //$sql1 .= " AND t.submitdate >= '" . date('Y-m-d') . "' group by t.ourrefno limit 0,100";
    
    //$my_paid_mail_date = "2020-11-06";  
    
    
    $sql .= " AND t.dt >= '" . $my_paid_mail_date . "' AND t.submitdate >= '" . date('Y-m-d') . "' group by t.ourrefno ORDER BY t.ourrefno ASC LIMIT 500";
    $sql1 .= " AND t.dt >= '" . $my_paid_mail_date . "' AND t.submitdate >= '" . date('Y-m-d') . "' group by t.ourrefno limit 0,100";
    
    //$my_paid_mail_date = "2024-07-26";
    //$sql .= " AND t.dt= '" . $my_paid_mail_date . "' AND t.submitdate >= '" . date('Y-m-d') . "' group by t.ourrefno ORDER BY t.ourrefno ASC LIMIT 500";
    //$sql1 .= " AND t.dt= '" . $my_paid_mail_date . "' AND t.submitdate >= '" . date('Y-m-d') . "' group by t.ourrefno limit 0,100";
    
    //echo $sql." ".$sql1; //print_r($paid_user_tender);
    //echo $u_val['user_id'];
    //echo $sql."<br/><br/>";die();
    ...
    /*$command = $connectionin->createCommand($sql);
    $dataReader = $command->query();
    $u_tender_listing = $dataReader->readAll();*/
    //echo $sql;die();
    //echo "<pre>";print_r($u_tender_listing);die();
...
$my_city_name = ucwords(trim($my_city_name));  
          ...
          $body .=' <tr>
              <td width="65%" style="border-right:1px solid #a69e9e;border-bottom:1px solid #002e3a;padding-bottom:5px">
                        ...
                        <table>
                          <tbody>
                            <tr>
                              ...
                              <td width="95%;" style="padding-top:10px;font-family:Arial,sans-serif">GCID : 
              ...
              <td style="width:20%;color:#41515e;text-align:center;border-bottom:1px solid #002e3a;padding-bottom:5px;border-right:1px solid #a69e9e">
              ...
              <td style="width:15%;color:#41515e;text-align:center;border-bottom:1px solid #002e3a;padding-bottom:5px">
                <p style="font-family:sans-serif;font-size:13px;font-stretch:normal;line-height:23px">
...
$body .=' </tbody>
    ...
    <tr>
      <td style="font-family:sans-serif;color:#464545;padding:15px 0 0 0">If you have any query or problem, please feel free to contact our support team:<br><br>You have received this mail from '.$sitename.' because you have subscribed to our Tender alert service.<br>
        <br>If you do not want to receive this mailer, 
         <a href="" style="color: #ed7e2c;" title="'.$sitename.'" target="_blank">Unsubscribe</a>
      </td>
    </tr>
...
//echo $body;die();
    ...
    $from_email = "info@tendebidalert.com";
    // More headers
    $headers .= 'From: info@tendebidalert.com' . "\r\n";
    $headers .= 'X-Mailer: PHP/'."\r\n";
    //$headers .= 'Cc: nationaltenderalert@gmail.com' . "\r\n";
    $headers .= 'Cc: '.$ccmailto. "\r\n";
    //$headers .= 'Bcc: nationaltenderalert@gmail.com' . "\r\n";
    ...
    if ($resultchk) {
      if (mysqli_num_rows($resultchk) > 0) {
        ...
        //echo 'not found';
            ...
            //$email_to = "patel.gautish2008@gmail.com,gautish.marvelinfo@gmail.com";
            //$status = mail($email_to, $subject, $body, $headers,"-odb -f $from_email"); //send mail

            //$status = mail($email_to, $subject, $body, $headers); //send mail
            //echo $body; die();

            /*****************mail send using smtp for domain email id***********************/
            $mailsent = email_phpmailer($email_recipients, $subject, $body,$emailUser,$emailPassword,$from,$fromname,$host, $log_file);
            /*****************mail send using smtp for domain email id***********************/
             
             //echo $email_sent_status;die();
            if ($email_sent_status == 1) {

                $tender_id = implode(',', $tender_array);
                $live_tender_count = 0;
                if($mailsent == "Message has been sent successfully"){

                $sql = "insert into userinfo (user_id,tender_date,status,is_live_tender_setting,total_fresh,total_live,support_executive,no_tender_available) values ('" . $u_val['user_id'] . "','" . date('Y-m-d') . "','1','0','" . $tender_count . "','" . $live_tender_count . "','0','0')";
        ...
        if ($count_remaining_send_email == 0) { 
                ...
                $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
                // More headers
                $headers .= 'From: '.$fromname.' <info@gem.com>' . "\r\n";
                //$headers .= 'Cc: myboss@example.com' . "\r\n";
                //$this_mail = mail($finish_email_to, $subject, $body,$headers);
                
                $email_recipients = array('gautish.marvelinfo@gmail.com');
                $mailsent = email_phpmailer($email_recipients, $subject, $body,$emailUser,$emailPassword,$from,$fromname,$host, $log_file);
...
// Optional: Logging a success message
//writeLog('Cron job executed successfully.', $log_file);
?>
