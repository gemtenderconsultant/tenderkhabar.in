...
// extends Model
{
    ...
    public function actionGettendersdata($name,$type){
        ...
        if(trim($keyword) != ""){
            ...
            if (!empty($my_sub) && $my_sub != '') {
                ...
                $sql = "SELECT t.ourrefno,t.purfromdate,t.Work,t.org_name,t.submitdate,t.earnestamount,t.doccost,t.tenderamount,Moneyformat(t.tenderamount) as ti_amount,DATE_FORMAT(t.submitdate, '%a %D %b, %Y') as show_ti_submit_date,t.city,t.org_name as agencyname,t.state_name as name,$searchtype from live_tenders as t INNER JOIN livetendercategory as tc ON t.ourrefno = tc.ourrefno where date(t.submitdate) >= '" . date('Y-m-d') . "'";
                ...
                //Moneyformat(t.tenderamount)
                $sql = "SELECT t.ourrefno,t.item,t.purfromdate,t.Work,t.org_name,t.submitdate,t.earnestamount,t.doccost,t.tenderamount,Moneyformat(t.tenderamount) as ti_amount,DATE_FORMAT(t.submitdate, '%a %D %b, %Y') as show_ti_submit_date,t.city,t.org_name as agencyname,t.state_name as name from live_tenders as t
            ...
            if($type == 'category' || $type == 'subcategory'){
                //Moneyformat(t.tenderamount)
                $sql = "SELECT t.ourrefno,t.purfromdate,t.Work,t.org_name,t.submitdate,t.earnestamount,t.doccost,t.tenderamount,Moneyformat(t.tenderamount) as ti_amount,DATE_FORMAT(t.submitdate, '%a %D %b, %Y') as show_ti_submit_date,t.city,t.org_name as agencyname,t.state_name as name from live_tenders as t 
    ...
    public function tendersearching($type){
        ...
        if(isset($userproduct['data']) && $userproduct['data'] == 'favourite'){
            if (Auth::check()) {
                //Moneyformat(t.tenderamount) 
                $sqlrecord = "SELECT t.TenderNo,t.ourrefno,t.purfromdate,t.Work,t.submitdate,t.earnestamount,t.doccost,t.org_name,t.tenderamount,Moneyformat(t.tenderamount) as ti_amount,DATE_FORMAT(t.submitdate, '%a %D %b, %Y') as show_ti_submit_date,t.city,t.org_name as agencyname,t.stateid,t.documentpath,t.state_name as name,tc.categoryid,tc.ourrefno,tc.subcategory,'India' as country,tl.user_id,tl.tender_id from $table_name as t
        ...
        if (!empty($userproduct['input_closingdate'])) {
            $closingdate = date('Y-m-d', strtotime($userproduct['input_closingdate']));
            $sql .= " AND DATE(t.submitdate) = '$closingdate'";
        }
    ...
    public function statesearching($id,$type){
        ...
        if(trim($keyword) != ""){
                    ...
                    if (count($sub_id) > 0) {
                       ...
                       if(Auth::check()){
                        ...
                        if (!empty($my_sub) && $my_sub != '') {
                            ...
                            $searchtype = "'Subcategory' AS searchtype";
                            //Moneyformat(t.tenderamount)
                            $sql = "SELECT t.ourrefno,t.purfromdate,t.Work,t.submitdate,t.earnestamount,t.org_name,t.doccost,t.tenderamount,Moneyformat(t.tenderamount) as ti_amount,DATE_FORMAT(t.submitdate, '%a %D %b, %Y') as show_ti_submit_date,t.city,t.org_name as agencyname,t.state_name as name,$searchtype from live_tenders as t INNER JOIN livetendercategory as tc ON t.ourrefno = tc.ourrefno where date(t.submitdate) >= '" . date('Y-m-d') . "'";
                    
                            }else{
                            $searchtype = "(CASE WHEN tc.subcategory = $my_sub THEN 'Subcategory' WHEN ($keyword_query) THEN 'BOQ' ELSE '' END) AS searchtype";
                            //Moneyformat(t.tenderamount)
                            $sql = "SELECT t.ourrefno,t.purfromdate,t.item,t.Work,t.submitdate,t.earnestamount,t.org_name,t.doccost,t.tenderamount,Moneyformat(t.tenderamount) as ti_amount,DATE_FORMAT(t.submitdate, '%a %D %b, %Y') as show_ti_submit_date,t.city,t.org_name as agencyname,t.state_name as name from live_tenders as t
                            INNER JOIN livetendercategory as tc ON t.ourrefno = tc.ourrefno where date(t.submitdate) >= '" . date('Y-m-d') . "'"; // ,$searchtype LEFT JOIN live_tenderinfo_items as lti ON t.ourrefno=lti.ourrefno
                            }
                            $sql1 = "SELECT COUNT(DISTINCT t.ourrefno) as total from live_tenders as t INNER JOIN livetendercategory as tc ON t.ourrefno = tc.ourrefno where date(t.submitdate) >= '" . date('Y-m-d') . "'"; // LEFT JOIN live_tenderinfo_items as lti ON t.ourrefno=lti.ourrefno
                            ...
                            $searchtype = "(CASE WHEN t.Work LIKE '%$keyword%' THEN 'Keyword' WHEN ($keyword_query) THEN 'BOQ' ELSE '' END) AS searchtype";
                            //Moneyformat(t.tenderamount)
                            $sql = "SELECT t.ourrefno,t.item,t.purfromdate,t.Work,t.submitdate,t.earnestamount,t.doccost,t.org_name,t.tenderamount,Moneyformat(t.tenderamount) as ti_amount,DATE_FORMAT(t.submitdate, '%a %D %b, %Y') as show_ti_submit_date,t.city,t.org_name as agencyname,t.state_name as name from live_tenders as t
                            where t.submitdate >= '" . date('Y-m-d') . "'"; // ,$searchtype LEFT JOIN live_tenderinfo_items as lti ON t.ourrefno=lti.ourrefno
           ...
           if(Auth::check()){    
            ...
            if($type == 'category' || $type == 'subcategory'){
                //Moneyformat(t.tenderamount)
                $sql = "SELECT t.ourrefno,t.purfromdate,t.Work,t.submitdate,t.earnestamount,t.doccost,t.org_name,t.tenderamount,Moneyformat(t.tenderamount) as ti_amount,DATE_FORMAT(t.submitdate, '%a %D %b, %Y') as show_ti_submit_date,t.city,t.org_name as agencyname,t.state_name as name from live_tenders as t 
                INNER JOIN livetendercategory as tc ON t.ourrefno = tc.ourrefno where date(t.submitdate) >= '" . date('Y-m-d') . "'";
                $sql1 = "SELECT COUNT(*) as total from live_tenders as t INNER JOIN livetendercategory as tc ON t.ourrefno = tc.ourrefno where date(t.submitdate) >= '" . date('Y-m-d') . "'"; 
            }else{
            //Moneyformat(t.tenderamount)
