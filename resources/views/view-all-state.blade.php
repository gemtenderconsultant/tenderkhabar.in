          ...
          <div class="container">
            <div class="row gy-4 d-flex justify-content-between">
              <div class="col-lg-12 order-2 order-lg-1 d-flex flex-column justify-content-center">
                ...
                <h2 style="text-align:center; padding-top: 20px; color:#1e3a8a;">All States</h2>
                <div class="search-wrapper">
                  <form id="searchForm" action="{{ route('state') }}">
                      <input type="text" id="searchInput" name="state" value="{{ request('state') }}" placeholder="Search state.." />
            ...
            <div class="card-grid">
                @if($state_data->count() > 0)
                    @foreach($state_data as $key => $value)
                        <div class="card-item" onclick="window.location.href='{{ route('stateresult', ['id' => $value->name]) }}'">
                            {{ $value->name }} Tenders
                        </div>
                    @endforeach
                @else
