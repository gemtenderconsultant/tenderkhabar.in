...
@section('content')   
    ...
    <header class="search-tender-header">
        ...
        <div class="search-tender-global-search">
            ...
            <input type="text" placeholder="Search tenders..." id="searchbox" value="{{ (isset($agencyname) && $agencyname !='') ? $agencyname  : '' }}" autocomplete="off" class="searchbox_input ui-autocomplete-input">
            </form>
        </div>
                    ...
                    @if(!empty($allEvents))
                        @foreach($allEvents as $key => $val)
                            @php 
                                ...
                                $res = isset($keyword) ? explode(',', $keyword) : [];
                                $sid = $val->ourrefno;
