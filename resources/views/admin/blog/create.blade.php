    ...
    <div class="row">
      <div class="col-md-12">
        <div class="card card-primary">
          ...
          <form name="Blogmeta-form" method="post" action="{{ route('blog-create') }}" enctype="multipart/form-data">
            <div class="card-body">
              <div class="form-group">
                 @csrf
                <label for="blog-name">Name</label>
                <input type="text" class="form-control {{ ($errors->has('name')) ? 'is-invalid' : '' }}" id="blog-name" name="name" placeholder="Enter Name" value="{{ old('name')}}">
                @if ($errors->has('name'))
              ...
              <div class="form-group">
                ...
                <textarea class="{{ ($errors->has('name')) ? 'is-invalid' : '' }}" id="description" name="description" row="5" placeholder="Enter Description">{{ old('description') }}</textarea>
              ...
              <div class="form-group">
                  ...
                  <select class="form-control {{ ($errors->has('category')) ? 'is-invalid' : '' }}" name="category" id="category">
                      ...
                      <option value="1" {{ (old('category')  == 1) ? "selected" : ''  }}>Tender</option>
                      <option value="2" {{ (old('category')  == 2) ? "selected" : ''  }}>GeM</option>
                  </select>
              ...
              <div class="form-group">
                ...
                <input type="text" class="form-control {{ ($errors->has('uploadby')) ? 'is-invalid' : '' }}" id="uploadby" name="uploadby" placeholder="Enter UploadBy" value="{{ old('uploadby') }}">
              ...
              <div class="form-group txt_image">
                    <label class="control-label" for="image">Image</label>
                    <input type="file" class="{{ ($errors->has('image')) ? 'is-invalid' : '' }}" name="image" value="{{old('image')}}">
                    @if ($errors->has('image'))
                      <span class="error invalid-feedback">{{ $errors->first('image') }}</span>
                    @endif
              </div>
              <div class="form-group txt_video {{ ($errors->has('youtube_url')) ? 'show' : 'hide' }}">
                  <label class="control-label" for="blog-youtube_url">Youtube Url</label>
                  <input type="text" name="youtube_url" class="form-control {{ ($errors->has('youtube_url')) ? 'is-invalid' : '' }}" value="{{ old('youtube_url') }}">
              ...
              <div class="form-group">
                  ...
                  <input type="text" name="tags" id="tags" value="{{ old('tags') }}" class="form-control {{ ($errors->has('tags')) ? 'is-invalid' : '' }}" placeholder="Enter Tags">
                  @if ($errors->has('tags'))
