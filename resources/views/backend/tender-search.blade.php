...
@section('content')
        ...
        <div class="tender-listing-filter-ribbon">
            ...
            <div class="tender-listing-filter-item">
                ...
                <div id="prefPanel" class="tender-listing-filter-panel" style="max-width: 350px;">
                    ...
                    <label class="tender-listing-checkbox-item" for="customCheck_{{ $tfval['id'] }}" data-text="{{ $tfval['filter_name'] }}" style="background: #f0f0f0; padding: 10px; border-radius: 8px;" onclick="togglePreferenceData()"
                        style="cursor:pointer;">
                        <input type="hidden" value="0" name="updatefiltermsg_{{ $tfval['id'] }}">
                        <input type="radio" {{ ($tfval['filter_name'] == "My Preference") ? "checked" : "" }} id="customCheck_{{ $tfval['id'] }}" data="{{ $tfval['id'] }}" data-main="{{ ($tfval['filter_name'] == "My Preference") ? 1 : 0 }}" name="example1" checked disabled>{{ $tfval['filter_name'] }} {{ ($tfval['filter_name'] == "My Preference") ? "(Not Editable)" : "" }}
                    <i data-lucide="chevron-down" size="16"></i>
                    </label>
                    <div id="preferenceData" style="display:none;">
                        <!-- Category -->
                    ...
                    <div class="tender-listing-panel-footer"><button class="tender-listing-btn-panel tender-listing-btn-apply">Apply Filter</button></div>
                </div>
                @endforeach
            </div>
            ...
            <div class="tender-listing-filter-item">
                <div class="tender-listing-filter-trigger dropdown-toggle" onclick="togglePanel('catPanel')">Category @if(isset($myuserproduct['subcategoryid']) && $myuserproduct['subcategoryid'] != '') @if(count(explode(',',$myuserproduct['subcategoryid'])) > 0) {{ '('.count(explode(',',$myuserproduct['subcategoryid'])).')'; }} @endif @endif @if(isset($type)) @if($type == "subcategory") {{ '(1)' }} @endif @endif <i data-lucide="chevron-down" size="14"></i></div>
                <div id="catPanel" class="tender-listing-filter-panel">
                    ...
                    <div class="tender-listing-checkbox-grid list_login_subcategory_list subcategory_scroll_div">
                    ...
                    <div class="tender-listing-panel-footer">
                        <button type="button" class="tender-listing-btn-panel tender-listing-btn-reset-panel btn_clear_selected_item btn_for_mobile" data="subcategory">Reset</button>
                        <button type="button"
                        class="tender-listing-btn-panel tender-listing-btn-apply sector_apply"
                        data="subcategory">
                        Apply Now
                        </button>
                    </div>
            ...
            <div class="tender-listing-filter-item">
                ...
                <div id="morePanel" class="tender-listing-filter-panel" style="max-width: 500px;">
                    ...
                    <div class="tender-listing-more-filters-form">
                        ...
                        <div class="tender-listing-form-row"><label>TKID Search</label><input type="text" class="clearallinput" id="Filters_ntid" placeholder="Search For GCID i.e : 26974461, 26974160" autocomplete="off"></div>
        ...
        <div style="display: flex; gap: 15px; align-items: center;">
            ...
            <div class="grid-dropdown-container">
                ...
                <div class="grid-dropdown-menu">
                    ...
                    <a href="javascript:void(0)" data="archive"class="dropdown-item">Archieve</a>
                    <a href="javascript:void(0)" data="archive2023"class="dropdown-item">Archieve-2023</a>
                    <a href="javascript:void(0)" data="archive2022"class="dropdown-item">Archieve-2022</a>
                    <a href="javascript:void(0)" data="archive2021"class="dropdown-item">Archieve-2021</a>
                    <a href="javascript:void(0)" data="archive2020"class="dropdown-item">Archieve-2020</a>
                </div>
            </div>
    ...
    <main class="tender-listing-content-area">
        <div id="Live" class="p-0 mt-3 tender_list">
            <div id="lev1"></div>
...
function togglePreferenceData()
{
    let box = document.getElementById('preferenceData');
...
<script type="text/javascript">
    ...
    @if(isset($page) && $page == "tenderlisting")
        offset = 1;
        busy = false;
        $("#lev1").html('');
        setTimeout(function () {
    ...
    $('body').on('click', '.dropdown-item', function () {
        var archive = $(this).attr('data');
        // set hidden value
        $('#archive_filter').val(archive);
        ...
        getFilter2(offset);// reload tenders
    });
    ...
    function getFilter2(offset) {
        ...
        var filter_scope = $('.sortfilter').val();
        var archive = $('#archive_filter').val();
        form_data.push({'name':'_token','value':$('meta[name="csrf-token"]').attr('content')});
        form_data.push({'name':'data','value':data});
        form_data.push({'name':'sortby','value':filter_scope});
        form_data.push({'name':'lpage','value':offset});
        form_data.push({'name':'archive','value':archive});
    
        $.ajax({
            'type': 'POST',
            'dataType': "json",
            'url': '{{ route("backendgettenderslist") }}',
            'data':form_data,
            'cache': false,
            beforeSend: function () {
    ...
    function filterChecks(input) {
        ...
        const panel = input.closest('.tender-listing-filter-panel');
        const labels = panel.querySelectorAll('.tender-listing-checkbox-grid .tender-listing-checkbox-item');
        labels.forEach(lbl => {
